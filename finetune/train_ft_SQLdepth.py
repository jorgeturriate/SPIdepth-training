...
#            pred = output["disp", 0]
            ...
            for i in range(pred.shape[0] // 2):
                ...
                # depth_np[np.isinf(depth_np)] = args.max_depth_eval
                # depth_np[np.isnan(depth_np)] = args.min_depth_eval
                # print(np.median(depth_np), np.median(pred_np)) # np.isnan() is not working
                if np.isnan(np.median(depth_np)) or np.isnan(np.median(pred_np)):
                    ratio = 1
                else:
                    ratio = np.median(depth_np) / np.median(pred_np)
                pred[i] *= ratio
            # ratio = np.median(depth[valid_mask].squeeze().detach().cpu().numpy()) / np.median(pred[valid_mask].squeeze().detach().cpu().numpy())
            # ratio = 0.002
            # pred *= ratio.detach()
