            ...
            if should_write and step % args.validate_every == 0:
                ...
                if should_log:
                    ...
                    model_io.save_checkpoint(model, optimizer, epoch, f"{experiment_name}_{run_id}_latest.pt",
                                             root=root) #root=os.path.join(root, "checkpoints"))
                ...
                if metrics['abs_rel'] < best_loss and should_write:
                    ...
                    save_folder = os.path.join(log_path, "models", f"weights_{epoch:02d}")
                    ...
                    print(f"Saved best model at epoch {epoch} -> {save_folder}")
                model.train()
                #################################################################################################
