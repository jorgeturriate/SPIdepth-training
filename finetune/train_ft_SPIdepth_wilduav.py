...
import model_io
import utils
from dataloader_wilduav import DepthDataLoader
...
#            pred = output["disp", 0]
            ...
            for i in range(pred.shape[0] // 2):
                ...
                depth_mask = np.logical_and(depth_np > args.min_depth_eval, depth_np < args.max_depth_eval)
                gt_height, gt_width = depth_np.shape

                if args.garg_crop or args.eigen_crop:
                    eval_mask = np.zeros_like(depth_np, dtype=bool)
                    if args.garg_crop:
                    ...
                    eval_mask = np.ones_like(depth_np, dtype=bool)
                valid_mask = np.logical_and(depth_mask, eval_mask)
...
#            pred = output["disp", 0]
            ...
            depth_mask = np.logical_and(gt_depth > args.min_depth_eval, gt_depth < args.max_depth_eval)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--notes", default='Finetuning SPIdepth on WildUAV dataset', type=str, help="Wandb notes")
    parser.add_argument("--tags", default='wilduav,finetuning', type=str, help="Wandb tags")
